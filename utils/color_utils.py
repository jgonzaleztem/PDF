...
def rgb_to_hex(rgb):
    ...
    Returns:
        str: Color en formato '#RRGGBB' o 'inválido'
    """
    if not isinstance(rgb, (tuple, list)) or len(rgb) != 3:
        logger.warning(f"rgb_to_hex recibió un valor no iterable o inválido: {rgb}")
        return "#000000"  # valor por defecto
    return '#{:02x}{:02x}{:02x}'.format(*rgb)

def normalize_color(color):
    """
    Convierte un valor de color en RGB (tuple) si es necesario.
    Acepta enteros, cadenas, o tuplas.
    """
    if isinstance(color, tuple) and len(color) == 3:
        return color
    elif isinstance(color, int):
        # Convertir enteros a escala de grises (0-255)
        return (color, color, color)
    elif isinstance(color, str):
        return extract_color(color)
    else:
        return None
